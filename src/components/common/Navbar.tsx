    ...
    return (
        <nav className="relative flex items-center justify-between px-6 md:px-12 py-6 bg-white shadow-sm z-50">
            ...
            <div className="hidden md:flex items-center gap-8">
                <Link to="/nosotros#nosotros" onClick={() => handleScrollLink('#nosotros')} className="text-gray-700 hover:text-brand transition-colors cursor-pointer">Acerca de</Link>
                <Link to="/nosotros#contacto" onClick={() => handleScrollLink('#contacto')} className="text-gray-700 hover:text-brand transition-colors cursor-pointer">Contáctanos</Link>
                
                {/* RENDERIZADO CONDICIONAL DE AUTH */}
                {!isAuthenticated ? (
                    <Link to="/login" className="bg-brand text-white px-6 py-2 rounded-xl font-medium hover:bg-brand-dark transition-all">
                        Iniciar sesión
                    </Link>
                ) : (
                    <div className="relative">
                        <button 
                            ...
                            className="flex items-center gap-2 text-dark font-medium hover:text-brand transition-colors focus:outline-none cursor-pointer"
                        >
            ...
            {isOpen && (
                <div className="absolute top-full left-0 w-full bg-white shadow-lg flex flex-col items-center py-8 gap-6 md:hidden border-t border-gray-100">
                    <Link to="/nosotros#nosotros" onClick={() => handleScrollLink('#nosotros')} className="text-gray-700 hover:text-brand text-lg font-medium">Acerca de</Link>
                    <Link to="/nosotros#contacto" onClick={() => handleScrollLink('#contacto')} className="text-gray-700 hover:text-brand text-lg font-medium">Contáctanos</Link>
                    
                    {!isAuthenticated ? (
                        <Link to="/login" onClick={toggleMenu} className="bg-brand text-white px-8 py-3 rounded-xl font-medium hover:bg-brand-dark transition-all w-10/12 text-center">
                            Iniciar sesión
                        </Link>
                    ) : (
                        <div className="w-full flex flex-col items-center gap-4 mt-2 border-t border-gray-100 pt-6">
                            ...
                            <button onClick={() => { handleLogout(); toggleMenu(); }} className="text-red-600 font-medium flex items-center gap-2 text-lg">
